...
class HrAttendancePolicy(models.Model):
    ...
    late_rule_id = fields.Many2one(
        comodel_name="hr.late.rule", required=True, string="Late In Rule"
    )
    absence_rule_id = fields.Many2one(
        comodel_name="hr.absence.rule", string="Absence Rule", required=True
    )
    diff_rule_id = fields.Many2one(
        comodel_name="hr.diff.rule", string="Difference Time Rule", required=True
    )
    ...
    def get_overtime(self):
        ...
        if self:
            ...
            if wd_ot_id:
                ...
                res["wd_rate_before"] = wd_ot_id.rate_before
                res["wd_before"] = wd_ot_id.active_before
            else:
                res["wd_rate"] = 1
                res["wd_after"] = 0
                res["wd_rate_before"] = 1
                res["wd_before"] = 0
            if we_ot_id:
                res["we_rate"] = we_ot_id.rate
                res["we_after"] = we_ot_id.active_after
                res["we_rate_before"] = we_ot_id.rate_before
                res["we_before"] = we_ot_id.active_before
            else:
                res["we_rate"] = 1
                res["we_after"] = 0
                res["we_rate_before"] = 1
                res["we_before"] = 0

            if ph_ot_id:
                res["ph_rate"] = ph_ot_id.rate
                res["ph_after"] = ph_ot_id.active_after
                res["ph_rate_before"] = ph_ot_id.rate_before
                res["ph_before"] = ph_ot_id.active_before
            else:
                res["ph_rate"] = 1
                res["ph_after"] = 0
                res["ph_rate_before"] = 1
                res["ph_before"] = 0

            if su_ot_id:
                res["su_rate"] = su_ot_id.rate
                res["su_after"] = su_ot_id.active_after
                res["su_rate_before"] = su_ot_id.rate_before
                res["su_before"] = su_ot_id.active_before
            else:
                res["su_rate"] = 1
                res["su_after"] = 0
                res["su_rate_before"] = 1
                res["su_before"] = 0

            if sa_ot_id:
                res["sa_rate"] = sa_ot_id.rate
                res["sa_after"] = sa_ot_id.active_after
                res["sa_rate_before"] = sa_ot_id.rate_before
                res["sa_before"] = sa_ot_id.active_before
            else:
                res["sa_rate"] = 1
                res["sa_after"] = 0
                res["sa_rate_before"] = 1
                res["sa_before"] = 0

        else:
            res["wd_rate"] = res["wd_rate"] = res["ph_rate"] = 1
            res["wd_after"] = res["we_after"] = res["ph_after"] = 0
            res["wd_rate_before"] = res["wd_rate_before"] = res["ph_rate_before"] = 1
            res["wd_before"] = res["we_before"] = res["ph_before"] = 0
        return res
...
class HrPolicy_overtimeLine(models.Model):
    ...
    overtime_rule_id = fields.Many2one(
        comodel_name="hr.overtime.rule", string="Name", required=True
    )
    type = fields.Selection(selection=type, string="Type", default="workday")
    active_after = fields.Float(
        string="Apply after", help="After this time the overtime will be calculated"
    )
...
class HrLateRule(models.Model):
    ...
    line_ids = fields.One2many(
        comodel_name="hr.late.rule.line",
...
class HrDiffRule(models.Model):
    ...
    name = fields.Char(string="name", required=True)
    line_ids = fields.One2many(
        comodel_name="hr.diff.rule.line",
...
class HrAbsenceRule(models.Model):
    ...
    name = fields.Char(string="name", required=True)
    line_ids = fields.One2many(
        comodel_name="hr.absence.rule.line",
